...
void smf_context::handle_itti_msg(
    ...
    // --- ORCHRA HOOK START ---
  if (this->get_custom_flag("migrating")) {
    Logger::smf_app().info("ORCHRA: Handling migration PFCP response for SEID " TEID_FMT, seresp.seid);

    uint32_t new_teid = 0;
    if (seresp.pfcp_ies.up_fseid.first) {
        // Extracting the TEID (s_eid) allocated by the target UPF
        new_teid = seresp.pfcp_ies.up_fseid.second.seid;
    }

    // Update the context with the new N4/N3 details
    this->set_teid_n3(new_teid); 
    this->set_custom_flag("migrated", true);
    this->set_custom_flag("migrating", false);

    // Trigger the N2 update to redirect the gNB
    // Note: We use shared_from_this() because create_n2... expects a shared_ptr
    smf_n2::get_instance().create_n2_pdu_session_resource_modify_request(shared_from_this());

    Logger::smf_app().info("ORCHRA: Migration N4 handshake complete. New TEID: %u. N2 Update triggered.", new_teid);
    return; // Stop here: don't look for standard procedures
  }
  // --- ORCHRA HOOK END ---
...
// ------------- Orchra --------------
void smf_context::set_upf(const std::string& upf_ip) {
    // 1. Prepare the Node ID (OAI's way of identifying UPFs)
    pfcp::node_id_t node_id = {};
    node_id.node_id_type = pfcp::NODE_ID_TYPE_IPV4_ADDRESS;
    
    // Convert string "10.0.0.1" to binary network format
    if (inet_aton(upf_ip.c_str(), &node_id.u1.ipv4_address) == 0) {
        Logger::smf_app().error("ORCHRA: Invalid IP format: %s", upf_ip.c_str());
        return;
    }

    // 2. Use the CORRECT singleton class name: pfcp_associations
    std::shared_ptr<pfcp_association> sa = nullptr;
    bool found = pfcp_associations::get_instance().get_association(node_id, sa);

    if (found && sa) {
        this->selected_upf = sa;
        Logger::smf_app().info("ORCHRA: Session anchored to UPF %s", upf_ip.c_str());
    } else {
        Logger::smf_app().warn("ORCHRA: Migration failed. UPF %s not found in active associations", upf_ip.c_str());
    }
}


std::string smf_context::get_upf() const {
  if (selected_upf && selected_upf->node_id.node_id_type == pfcp::NODE_ID_TYPE_IPV4_ADDRESS) {
    return inet_ntoa(selected_upf->node_id.u1.ipv4_address);
  }
  return "none";
}
...
//std::string smf_context::get_upf() const {
//  if (selected_upf) {
//    return selected_upf->node_id.u1.ipv4_address;
//  }
//  return "none";
//}

void smf_context::set_custom_flag(const std::string& key, bool value) {
    if (key == "migrated") is_migrated = value;
  // orchra_flags_[key] = value;
}

bool smf_context::get_custom_flag(const std::string& key) const {
  //auto it = orchra_flags_.find(key);
  // return it != orchra_flags_.end() ? it->second : false;
  if (key == "migrated") return is_migrated;

  return false;
}

/*
void smf_context::mark_as_migrated(const std::shared_ptr<oai::app::smf::pfcp_association>& new_upf, uint32_t new_teid) {
    this->set_selected_upf(new_upf);
    this->set_teid_n3(new_teid); // Store the TEID provided by the new UPF
    this->set_custom_flag("migrated", true);

    char ip_str[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &(new_upf->node_id.u1.ipv4_address), ip_str, INET_ADDRSTRLEN);
    this->set_selected_upf_n3_ip(ip_str);
}
*/

void smf_context::set_selected_upf_node_id(const std::string& v) { selected_upf_node_id_ = v; }
void smf_context::set_selected_upf_n3_ip(const std::string& v)   { selected_upf_n3_ip_ = v; }
void smf_context::set_selected_upf_n4_addr(const std::string& v) { selected_upf_n4_addr_ = v; }

const std::string& smf_context::get_selected_upf_node_id() const { return selected_upf_node_id_; }
const std::string& smf_context::get_selected_upf_n3_ip() const   { return selected_upf_n3_ip_; }
const std::string& smf_context::get_selected_upf_n4_addr() const { return selected_upf_n4_addr_; }
