...
smf_config::smf_config(
    ...
    : config(configPath, oai::config::SMF_CONFIG_NAME, logStdout, logRotFile),
      ...
      itti() {
  // -------------- Orchra --------------
  m_local_config_path = configPath;
...
void smf_config::to_smf_config() {
  ...
  // DNNs
  std::cout << "DNN catalog is size: " << get_dnns().size() << std::endl;
...
bool smf_config::init() {
  ...
  try {
      ...
      Logger::smf_app().warn("Could not find internal_api block: %s", e.what());
      std::cerr << "Exception in DNN loop: " << e.what() << std::endl;
      display(); // Show what we have before crashing
      // throw; return false;
  }
  ...
  // 4. --- ORCHRA RELOAD ---
  try {
    // 1. Use the getter get_config_path() instead of the private variable
    YAML::Node orchra_node = YAML::LoadFile(m_local_config_path); 

    if (orchra_node["smf"] && orchra_node["smf"]["internal_api"]) {
    ...
    Logger::smf_app().warn("Orchra: Failed to parse custom config: %s", e.what());
  }
...
void smf_config::load_internal_api(const YAML::Node& node) {
    try {
        // 1. Parse Redis Configuration
        if (node["redis_host"]) {
            std::string r_host = node["redis_host"].as<std::string>();
            int r_port = node["redis_port"] ? node["redis_port"].as<int>() : 6379;
            internal_api.redis_url = "tcp://" + r_host + ":" + std::to_string(r_port);
        } else {
            internal_api.redis_url = "tcp://127.0.0.1:6379"; // Default fallback
        }

        // 2. Parse Bind Address (Replacing 'host' with 'bind_address' from your header)
        if (node["bind_address"]) {
            internal_api.bind_address = node["bind_address"].as<std::string>();
        } else {
            internal_api.bind_address = "0.0.0.0"; // Default
        }

        // 3. Parse API Port
        if (node["port"]) {
            internal_api.port = node["port"].as<int>();
        } else {
            internal_api.port = 8080; // Default
        }

        internal_api.enabled = true;

        // 4. Log configuration (Using bind_address to match struct member)
        Logger::smf_app().info("Orchra Internal API Config: %s:%d (Redis: %s)",
                                internal_api.bind_address.c_str(),
                                internal_api.port,
                                internal_api.redis_url.c_str());

    } catch (const std::exception& e) {
        Logger::smf_app().error("Failed to parse Internal API block: %s", e.what());
        throw; 
    }   
}
} // namespace oai::config::smf
