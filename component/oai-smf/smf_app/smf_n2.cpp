...
extern oai::app::smf::smf_sbi* smf_sbi_inst;
...
bool smf_n2::create_n2_pdu_session_resource_modify_request_transfer(
  ...
  // --- ORCHRA HOOK START ---
  // 1. Look up the context via the SMF App Singleton
  extern smf_app* smf_app_inst;

  // 1. Get SMF context using SUPI
  std::shared_ptr<smf_context> sc =
      smf_app_inst->get_smf_context(sm_context_res->get_supi());

  if (!sc) {
      Logger::smf_n2().error("ORCHRA: Could not find smf_context for SUPI %s", sm_context_res->get_supi().c_str());
      return false;
  }
  ...
  if (sc->get_custom_flag("migrated")) {
       ...
       std::string new_upf_ip = sc->get_selected_upf_n3_ip();						       
       
       uint32_t dl_teid = sc->get_teid_n3();
       std::shared_ptr<smf_pdu_session> psc = nullptr; // {};
       std::cout<<"\t *****************8 "<<dl_teid <<std::endl;						 

       if (!qos_flows.empty()) {
           // Get the TEID from the first flow's DL F-TEID
           dl_teid = qos_flows.begin()->second.dl_fteid.teid;
       }

       if (selected_upf_ptr) {
           struct in_addr n3_inet_addr = selected_upf_ptr->node_id.u1.ipv4_address;
           char buf[INET_ADDRSTRLEN];
           //inet_ntop(AF_INET, &(selected_upf_ptr->node_id.u1.ipv4_address), buf, INET_ADDRSTRLEN);
           std::string upf_ip(buf);
      
           Logger::smf_app().info("ORCHRA: Migration hook active. Overriding N2 info with UPF IP: %s", upf_ip.c_str());

	   // 1. Convert string IP to struct in_addr for the NGAP IE
           // struct in_addr n3_inet_addr;
           inet_aton(upf_ip.c_str(), &n3_inet_addr);

	   // 2. Correct TEID (DL TEID from the UPF context)
           // uint32_t teid_val = sc->get_selected_upf_dl_teid(); 

           // Create Helper Objects for NGAP
           oai::ngap::TransportLayerAddress tla = {};
	   // uint32_t addr = n3_inet_addr.s_addr;
           tla.setIpv4Address(n3_inet_addr); 

           oai::ngap::GtpTeid teid = {};
	   
           // Convert string TEID to uint32_t (assuming it's a numeric string)
           try {
               // uint32_t teid_val = std::stoul(sc->get_selected_upf_node_id());
	       // uint32_t teid_val = sc->get_selected_upf_dl_teid();
               teid.set(dl_teid);
           } catch (std::exception& e) {
               Logger::smf_n2().error("ORCHRA: Invalid TEID string: %s", sc->get_selected_upf_node_id().c_str());
           }
 
           oai::ngap::UpTransportLayerInformation utli;
           utli.set(tla, teid);

           oai::ngap::UlNgUUpTnlModifyItem modify_item;
           modify_item.setUlNgUUpTnlInformation(utli);

           oai::ngap::UlNgUUpTnlModifyList modify_list;
           modify_list.addItem(modify_item); 

           pdu_session_resource_modify_request_transfer.setUlNgUUpTnlModifyList(modify_list);

           // Logger::smf_app().info("ORCHRA: Successfully set Migrate UPF TNL info");
	   Logger::smf_n2().info("ORCHRA: N2 reflects migrated state: IP %s, TEID %u",
                          new_upf_ip.c_str(), dl_teid);

	   sc->set_custom_flag("migrated", true);
	   sc->set_custom_flag("migrating", false);
       }
    } else {
	   // --- ORIGINAL OAI LOGIC ---
      std::shared_ptr<smf_pdu_session> psc = {};
      if (sc->find_pdu_session(sm_context_res->get_pdu_session_id(), psc)) {
	   struct in_addr n3_inet_addr;
           inet_aton(sc->get_selected_upf_n3_ip().c_str(), &n3_inet_addr);

           oai::ngap::TransportLayerAddress tla = {};
           tla.setIpv4Address(n3_inet_addr); // sc->get_selected_upf_n3_ip());
    
           oai::ngap::GtpTeid teid = {};
           try {
                teid.set(std::stoul(sc->get_selected_upf_node_id(), nullptr, 10));
           } catch (const std::exception& e) {
               Logger::smf_n2().error("TEID conversion failed: %s", e.what());
	   }
	    ...
	    sc->set_custom_flag("migrating", false);
            ...
            // pdu_session_resource_modify_request_transfer.set(utli); //UPTransportLayerInformation(utli);    
      }
  }
...
//------------------------------------------  Orchra ------------------------------------------
bool smf_n2::create_n2_pdu_session_resource_modify_request(const std::shared_ptr<smf_context>& sc) {
    std::string ngap_msg;

    pdu_session_update_sm_context_response res; 
    res.set_supi(sc->get_supi());

    // smf_sbi* smf_sbi_inst = &smf_sbi::get_instance();
    // extern smf_sbi* smf_sbi_inst;
    
    // uint8_t pdu_session_id = 0;
    // auto& sessions = sc->get_pdu_sessions();
    ...
    //if (!sessions.empty()) {
    //    pdu_session_id = sessions.begin()->first;
    //}

    uint8_t pdu_sid = sessions.begin()->first;
    res.set_pdu_session_id(pdu_sid);
    // res.set_supi(sc->get_supi());
    // res.set_pdu_session_id(sc->get_pdu_session_id());

    // 1. Generate the NGAP binary data
    bool success = create_n2_pdu_session_resource_modify_request_transfer(
        res, 
        n2_sm_info_type_e::PDU_RES_MOD_REQ, //PDU_SESSION_RESOURCE_MODIFY_REQUEST_TRANSFER, 
        ngap_msg
    );

    if (success && !ngap_msg.empty()) {
        Logger::smf_n2().info("ORCHRA: NGAP payload ready. Packaging into ITTI NX msg...");

        // 2. Create the ITTI message
        // Note: TASK_SMF_APP as origin, TASK_SMF_SBI as destination
        auto itti_msg = std::make_shared<itti_nx_trigger_pdu_session_modification>(TASK_SMF_APP, TASK_SMF_SBI);

        // 3. Fill the nested 'msg' object
        itti_msg->msg.set_supi(sc->get_supi());
	itti_msg->msg.set_pdu_session_id(pdu_sid); //pdu_session_id);
        // itti_msg->msg.set_pdu_session_id(sc->get_pdu_session_id());
	
        
        // This puts the NGAP binary data into the message
        itti_msg->msg.set_n2_sm_information(ngap_msg);
        itti_msg->msg.set_n2_sm_info_type("PDU_RES_MOD_REQ");
        
        // Set HTTP version (usually 1 for HTTP/1.1 or 2 for HTTP/2)
        itti_msg->http_version = 1; 

        // 4. Send it to SBI
	smf_sbi_inst->send_n1n2_message_transfer_request(itti_msg);
        // smf_sbi::get_instance().send_n1n2_message_transfer_request(itti_msg);
