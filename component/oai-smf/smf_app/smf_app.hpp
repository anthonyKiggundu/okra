...
// Forward declare the API Impl
namespace oai::smf_server::api {
    class ContextTransferApiImpl;
}
...
struct ImportedQosRule {
    pfcp::qfi_t qfi;
    oai::nas::QosRule rule;
};

using context_key_t = std::pair<std::string, uint32_t>;
...
struct imported_pdu_context {
    ...
    std::string amf_uri; // Needed for signaling
    std::vector<ImportedQosRule> qos_rules;
...
// std::map<context_key_t, std::shared_ptr<imported_pdu_context>> m_imported_ctx;
...
class smf_app {
 private:
  ...
  // Orchra
  // std::unique_ptr<ContextTransferApiImpl> m_internal_api_server;
  std::shared_ptr<Pistache::Rest::Router> m_api_server_rest_router;
  std::unique_ptr<oai::smf_server::api::ContextTransferApiImpl> m_internal_api_server;
  std::map<context_key_t, std::shared_ptr<imported_pdu_context>> m_imported_ctx;
  std::mutex m_imported_ctx_mutex;
  std::map<std::string, std::shared_ptr<smf_context>> m_smf_contexts;

  // Helper to store the AMF URI temporarily if needed during the async flow
  // (Alternatively, you can store this inside the smf_pdu_session object)
  std::map<uint64_t, std::string> m_restoration_amf_uris;
 ...
 public:
  ...
  // -------------- orchra ---------------------
  std::map<std::string, std::shared_ptr<upf_node>> upf_nodes; 

  bool send_n4_msg(const std::shared_ptr<pfcp_association>& sa,
  ...
  get_imported_pdu_context(const std::string& supi, uint32_t pdu_id) const;
  ...
  std::shared_ptr<upf_node> resolve_upf_node(const std::string& node_id) const;

  bool is_same_physical_upf(const std::string& addr1, const std::string& addr2) const;

  // Pause / Resume
  bool pause_pdu_session(const std::string& supi, uint32_t pdu_id);
  bool resume_pdu_session(const std::string& supi, uint32_t pdu_id, uint32_t teid, const std::string& ipv4);

  // bool resume_pdu_session(const std::string& supi, uint32_t pdu_id, const oai::model::smf::UpfResumeRequest &body, Pistache::Http::ResponseWriter &response);
  // bool resume_pdu_session(const std::string& supi, const int32_t& pduSessionId, const nlohmann::json &body, Pistache::Http::ResponseWriter& response);

  // Export / Import
  bool export_pdu_session_context(const std::string& supi,uint32_t pdu_id, nlohmann::json& dto);

  // bool import_pdu_session_context(const std::string& supi, uint32_t pdu_id, const nlohmann::json& data);
  bool import_pdu_session_context(const imported_pdu_context& data, Pistache::Http::ResponseWriter& response); //ImportedSessionData& data);

  bool export_pdu_session_context(const std::string& imsi, uint32_t pduSessionId, std::shared_ptr<imported_pdu_context> ctx);

  // Binding / Routing
  bool bind_pdu_session_to_upf(const std::string& supi, uint32_t pdu_id);

  bool bind_inter_upf(const std::string& supi, uint32_t pdu_id);

  // Release
  bool release_pdu_session(const std::string& supi, uint32_t pdu_id);

  // N2 signaling
  bool send_n2_pdu_session_update(std::shared_ptr<imported_pdu_context> ctx);

  std::shared_ptr<smf_context> get_smf_context(const std::string& supi);

  std::shared_ptr<oai::app::smf::smf_pdu_session> stage_pdu_session_context(const imported_pdu_context& data); //ImportedSessionData& data);

  bool commit_pdu_session_context_async(std::shared_ptr<oai::app::smf::smf_pdu_session> session);

  void handle_n4_restoration_response(const std::shared_ptr<itti_n4_session_establishment_response>& msg);

  bool commit_restored_session(const std::shared_ptr<oai::app::smf::smf_pdu_session>& session,  const std::shared_ptr<oai::app::smf::pfcp_association>& sa,
  ...
  bool trigger_n1n2_resync(const std::shared_ptr<oai::app::smf::smf_pdu_session>& session, const std::shared_ptr<oai::app::smf::smf_context>& sc);

  bool build_restoration_pfcp_request(const std::shared_ptr<smf_pdu_session>& session,  const imported_pdu_context& data, pfcp::pfcp_session_establishment_request& req);

  // Add any other missing helpers you wrote like:
  uint32_t allocate_teid();
  uint64_t generate_new_seid();
   // ============= end of orchra ===================
};
}  // namespace oai::app::smf
