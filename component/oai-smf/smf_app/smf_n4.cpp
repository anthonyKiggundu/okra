...
void smf_n4::handle_receive_session_establishment_response(
  ...
  Logger::smf_n4().info(
    "ORCHRA-DBG N4 rx %s trxn=%lu seid=" SEID_FMT " remote=%s",
    "SESSION_DELETION_RESPONSE", trxn_id, msg.get_seid(),
    remote_endpoint.toString().c_str());

  handle_receive_message_cb(msg, remote_endpoint, TASK_SMF_N4, error, trxn_id);
  if (!error) {
    // ------------------ begin of Orchra ------------------
    if (!msg_ies_container.cause.first) {
      Logger::smf_n4().warn(
          "Received N4 SESSION ESTABLISHMENT RESPONSE without cause IE, ignore message");
      return;
    }
    // ------------------ end of Orchra ------------------
...
void smf_n4::handle_receive_session_modification_response(
  ...
  if (!error) {
    ...
    if (!msg_ies_container.cause.first) {
      Logger::smf_n4().warn(
          "Received N4 SESSION MODIFICATION RESPONSE without cause IE, ignore message");
      return;
    }
    // --------------------- end of Orchra -------------------
...
void smf_n4::handle_receive_session_deletion_response(
  ...
  if (!error) {
    ...
    if (!msg_ies_container.cause.first) {
      Logger::smf_n4().warn(
          "Received N4 SESSION DELETION RESPONSE without cause IE, ignore message");
      return;
    }
    // --------------------- end of Orchra ----------------
...
void smf_n4::handle_receive(
  ...
  try {
    ...
    Logger::smf_n4().error("handle_receive std::exception %s", e.what());
  } catch (...) {
    Logger::smf_n4().error("handle_receive unknown exception");
  }
}
