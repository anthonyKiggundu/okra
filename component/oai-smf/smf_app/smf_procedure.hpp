...
class smf_procedure { 
 private:	 
  ...
  // ----------------- orchra ---------------------
  uint16_t next_pdr_rule_id = 1; // Start from 1 per 3GPP practice
  // ----------------- end of orchra --------------
 ...
 public:  
  ...
  // ------------------ orchra ---------------------
  pfcp::pdr_id_s generate_pdr_id() {
    pfcp::pdr_id_s pdr_id;
    pdr_id.rule_id = next_pdr_rule_id++; // Thread safety: add mutex if needed
    return pdr_id;
  }

  bool trigger_upf_migration(
    const std::shared_ptr<smf_context>& sc,
    const std::shared_ptr<oai::app::smf::pfcp_association>& target_upf);

  bool build_migration_pfcp_session_request(
    const std::shared_ptr<smf_context>& sc,
    const std::shared_ptr<oai::app::smf::pfcp_association>& target_upf,
    //oai::app::smf::pfcp_session_establishment_request& req);
    pfcp::pfcp_session_establishment_request& req);

  void build_orchra_rules(std::shared_ptr<smf_context> ctx);
...
class session_update_sm_context_procedure : public smf_session_procedure {
 public:
  ...
  // OKRA:: Public wrapper so external triggers (e.g., REST “pause/resume”) can force an N4 mod.
  smf_procedure_code 
  trigger_n4_modification_for_qfis(
      const std::vector<pfcp::qfi_t>& list_of_qfis);
