...
// ------------------- Orchra -----------------------
#include "orchra_pfcp.hpp" // For fill_pfcp_from_snapshot
#include "smf_n4.hpp"      // For smf_n4::get_instance()
#include "common_defs.h"   // For DISPATCH_MESSAGE_OK
#include "pfcp.hpp"
#include "3gpp_29.244.h"   // For pdr_id_s rule_id
// ------------------- end of orchra ----------------
...
pfcp::create_far smf_session_procedure::pfcp_create_far(
  ...
  // After forwarding_parameters.set(destination_interface);
  // and before create_far.set(apply_action);  
  if (!edge->uplink && sps && sps->dl_buffering_paused.load()) {
    // Downlink FAR: buffer instead of forward
    apply_action.forw = 0;
    apply_action.buff = 1;
    apply_action.drop = 0;
    apply_action.nocp = 0;
  ...
  Logger::smf_app().info(
    "pfcp_create_far: edge uplink=%d qfi=%u paused=%d -> apply_action(forw=%d,buff=%d,drop=%d) teid=0x%x ip=%s",
    edge->uplink ? 1 : 0,
    edge->qfi.qfi,
    (sps && sps->dl_buffering_paused.load()) ? 1 : 0,
    apply_action.forw,
    apply_action.buff,
    apply_action.drop,
    edge->next_hop_fteid.teid,
    inet_ntoa(edge->next_hop_fteid.ipv4_address));
...
pfcp::create_pdr smf_session_procedure::pfcp_create_pdr(
  ...
  // Packet detection infrmation (see Table 7.5.2.2-2: PDI IE within PFCP
...
smf_procedure_code
	...
	smf_procedure_code session_update_sm_context_procedure::run(
	  ...
	  // -------------------- Orchra --------------------
	  Logger::smf_app().info("Running PDU Session Update SM Context Procedure...");

          // Check if our Redis hook marked this context
          if (sc && sc->get_custom_flag("migrated")) {
              Logger::smf_app().info("ORCHRA: Forced migration detected for %s", sc->get_supi().c_str());

              // Use getter instead of private member
              auto target_upf = sc->get_selected_upf(); 

              if (target_upf) {
                  // 2. Prepare the Request Object
                  pfcp_session_modification_request req = {};

                  // 3. Populate Snapshot (Ensure orchra.hpp is included at the top)
                  // If you don't have a namespace, use: fill_pfcp_from_snapshot(sc, req);
                  orchra::fill_pfcp_from_snapshot(sc, req);
 
                  // 4. Trigger N4. In this procedure class, we usually fill 'n4_triggered' 
                  // and let the base logic send it, or call the QFI-based version.
                  std::vector<pfcp::qfi_t> qfis;
                  sm_context_req->req.get_qfis(qfis);
                  this->send_n4_session_modification_request(qfis);
 
                  // 5. Correct IP Logging
                  Logger::smf_app().info("ORCHRA: N4 Modification sent to target UPF: %s",
                      oai::utils::conv::toString(target_upf->node_id.u1.ipv4_address).c_str());
              }
          } 
	  // ------------------- end of Orchra -------------
...
// -------------- Orchra -------------------------
/*
void smf_procedure::build_orchra_rules(std::shared_ptr<smf_context> ctx) {
    // 1. Packet Detection Rule (PDR) - Catch Uplink from gNB
    pfcp::pdr_t pdr_ul = {};
    pdr_ul.pdr_id = 1;
    pdr_ul.precedence = 100;
    // Set the TEID we imported or allocated
    pdr_ul.pdi.teid = ctx->get_teid_ul(); 
    
    // 2. Forwarding Action Rule (FAR) - Send to Internet (N6)
    pfcp::far_t far_ul = {};
    far_ul.far_id = 1;
    far_ul.apply_action.forw = 1;
    far_ul.forwarding_parameters.destination_interface = pfcp::destination_interface_t::SGI_LAN_N6_LAN;

    // 3. Send to the NEW UPF
    if (ctx->selected_upf) {
        smf_pfcp_association::get_instance().send_session_establishment_request(
            ctx->selected_upf, ctx, pdr_ul, far_ul);
    }
}


bool smf_procedure::trigger_upf_migration(
    const std::shared_ptr<smf_context>& sc,
    const std::shared_ptr<oai::app::smf::pfcp_association>& target_upf) {

  if (!sc || !target_upf) return false;

  Logger::smf_app().info("ORCHRA: Starting migration for SUPI %s to UPF %s",
                         sc->get_supi().c_str(), target_upf->node_id.toString().c_str());

  // 1. Build the PFCP Establishment Request by cloning current session state
  // We use Establishment because the target UPF does not have this session yet.
  oai::app::smf::pfcp_session_establishment_request req = {};

  // 2. Clone state from existing session
  // Look for OAI's existing "fill_pfcp_session_establishment_request"
  std::map<uint8_t, std::shared_ptr<smf_pdu_session>> sessions;
  sc->get_pdu_sessions(sessions);

  for (auto const& [s_id, session] : sessions) {
      for (auto const& [p_id, pdr] : session->pdrs) {
          pfcp::pdr_t cloned_pdr = pdr; 
          req.create_pdrs.push_back(cloned_pdr);
      }
  }

  if (!build_migration_pfcp_session_request(sc, target_upf, req)) {
      Logger::smf_app().error("ORCHRA: Failed to clone session state for PFCP");
      return false;
  }

  // 2. Send via the existing N4 stack
  // This uses the standard OAI smf_n4 instance to handle sequence numbers and retransmissions
  int res = smf_n4::get_instance().send_pfcp_session_establishment_request(target_upf, sc, req);

  if (res != DISPATCH_MESSAGE_OK) {
      Logger::smf_app().error("ORCHRA: Failed to dispatch PFCP migration request to N4 stack");
      return false;
  }

  // Note: In a production environment, you would wait for the PFCP Response
  // in a callback before proceeding. For this adjustment, we assume synchronous success
  // or that the state is marked upon receipt of the N4 Response.
...
bool smf_procedure::trigger_upf_migration(
  ...
  Logger::smf_app().info("Starting migration for SUPI %s to UPF %s",
                         sc->get_supi().c_str(), target_upf->node_id.toString().c_str());

  // 1. OAI uses a specific namespace for the request
  //oai::app::smf::pfcp_session_establishment_request req = {};
  pfcp::pfcp_session_establishment_request req = {};
  // auto n4_inst = smf_app_inst->get_n4();

  // 2. Build the request using the helper below
  if (!build_migration_pfcp_session_request(sc, target_upf, req)) {
      Logger::smf_app().error("Failed to clone session state for PFCP");
      return false;
  }

  Logger::smf_app().info(
      "Built PFCP migration request for SUPI %s toward UPF %s",
      sc->get_supi().c_str(),
      target_upf->node_id.toString().c_str());

  // return true;
  
  auto n4_msg = std::make_shared<itti_n4_session_establishment_request>(
      TASK_SMF_APP, TASK_SMF_N4);
  n4_msg->pfcp_ies = req;

  if (target_upf->node_id.node_id_type != pfcp::NODE_ID_TYPE_IPV4_ADDRESS) {
    Logger::smf_app().error("Target UPF has no valid IPv4 node ID");
    return false;
  }

  uint16_t pfcp_port = 8805;  
  n4_msg->r_endpoint =
      endpoint(target_upf->node_id.u1.ipv4_address, pfcp_port); // PFCP_N4_PORT

  // You need the existing helper that actually dispatches the ITTI message.
  // In your tree this is used from smf_app.cpp, not from smf_n4.
  return smf_app_inst->send_n4_msg(target_upf, n4_msg);

}

/*
bool smf_procedure::build_migration_pfcp_session_request(
    const std::shared_ptr<smf_context>& sc,
    const std::shared_ptr<oai::app::smf::pfcp_association>& target_upf,
    pfcp::pfcp_session_establishment_request& req) {
  ...
  auto psc = sessions.begin()->second; // Use the first active session

  req.node_id = target_upf->node_id;
  // Use get_cp_fseid() if available, or access via sc
  // req.cp_fseid = sc->get_cp_fseid(); 

  for (auto& it_pdr : psc->pdrs) {
      pfcp::pdr_t cloned_pdr = it_pdr.second;
      if (cloned_pdr.pdi.local_fteid.first) {
          cloned_pdr.pdi.local_fteid.second.ipv4_address = target_upf->node_id.u1.ipv4_address;
      }
      req.create_pdrs.push_back(cloned_pdr);
  }

  for (auto& it_far : psc->fars) {
      req.create_fars.push_back(it_far.second);
  }

  // 4. Clone QERs (QoS Enforcement Rules)
  for (auto& it_qer : psc->qers) {
      req.create_qers.push_back(it_qer.second);
  }
...
bool smf_procedure::build_migration_pfcp_session_request(
  ...
  sc->get_pdu_sessions(sessions);
  if (sessions.empty()) return false;
  auto psc = sessions.begin()->second;

  // Set Node ID
  req.node_id.first = true;
  req.node_id.second = target_upf->node_id;
  ...
  for (const auto& qfi_item : all_qfis) {
    auto edge = sh->public_get_edge_for_qfi(qfi_item.qfi);
    if (!edge) continue;

    // --- BUILD PDR ---
    // pfcp::pdr pdr_item = {};
    pfcp::create_pdr pdr_item = {};
    pdr_item.pdr_id.first = true;
    pdr_item.pdr_id.second = edge->pdr_id;
    
    pdr_item.precedence.first = true;
    pdr_item.precedence.second.precedence = edge->precedence;

    // Set PDI (Packet Detection Information)
    pdr_item.pdi.first = true;
    pdr_item.pdi.second.source_interface.first = true;
    pdr_item.pdi.second.source_interface.second.interface_value =
        edge->uplink ? pfcp::INTERFACE_VALUE_ACCESS : pfcp::INTERFACE_VALUE_CORE;
	   // = (edge->uplink) ? 0 : 1; // 0: Access, 1: Core

    // Local F-TEID: Update to the new target UPF IP
    pdr_item.pdi.second.local_fteid.first = true;
    pdr_item.pdi.second.local_fteid.second = edge->fteid;
    pdr_item.pdi.second.local_fteid.second.ipv4_address = target_upf->node_id.u1.ipv4_address;

    // IDs linking
    pdr_item.far_id.first = true;
    pdr_item.far_id.second = edge->far_id;
    // pdr_item.qer_id.push_back({true, edge->qer_id});
    pdr_item.qer_id.first = true;
    pdr_item.qer_id.second = edge->qer_id;

    req.create_pdrs.push_back(pdr_item);

    // --- BUILD FAR ---
    // pfcp::far_t far_item = {};
    pfcp::create_far far_item = {};
    far_item.far_id.first = true;
    far_item.far_id.second = edge->far_id;

    far_item.apply_action.second.forw = true;
    far_item.apply_action.second.buff = false;

    req.create_fars.push_back(far_item);
  }

  return (!req.create_pdrs.empty());
}
...
void smf_procedure::build_orchra_rules(std::shared_ptr<smf_context> ctx) {
  ...
  for (const auto& [pdu_id, psc] : sessions) {
    auto sh = psc->get_session_handler();
    if (!sh) continue;
    ...
    for (const auto& qfi_item : all_qfis) {
      ...
      // FIX: Use .pdr_id and .far_id instead of .rule_id
      if (edge->pdr_id.rule_id == 0) {
          edge->pdr_id = sh->generate_pdr_id();
      }
      
      if (edge->far_id.far_id == 0) {
          edge->far_id = sh->generate_far_id();
      }

      Logger::smf_app().info(
          "Orchra: PDU %u QFI %u -> PDR %u, FAR %u",
          pdu_id, qfi_item.qfi, edge->pdr_id.rule_id, edge->far_id.far_id);
    }
  }
}
// -------------end of orchra -------------------
