...
void SMFApiServer::init(size_t thr) {
  ...
  m_smfConfigurationApiImpl->init();
  m_contextTransferApiImpl->init();
}
...
// Orchra
// Assuming Pistache or similar router
// These routes have to be generated automatically by the OAI API
/*router->post("/internal/sm-contexts/:imsi/:pduSessionId/pause",
             [this](const Http::Request& req, Http::ResponseWriter resp) {
                 auto imsi = req.param(":imsi").as<std::string>();
                 auto pdu_id = std::stoi(req.param(":pduSessionId").as<std::string>());
                 pause_pdu_session(imsi, pdu_id, resp);
             });
*/
