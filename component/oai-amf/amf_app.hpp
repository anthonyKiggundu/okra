...
class amf_app {
 private:
  ...
  // Orchra extensions are till end of private section
  // amf_config amf_cfg;

  // Detached imported contexts (keyed by SUPI/ueId)
  std::map<std::string, nlohmann::json> m_detached_ue_contexts;
  mutable std::shared_mutex m_detached_ue_contexts_mutex;
 ...
 public:
  ...
  // Orchra extension till end of public section
  bool internal_export_ue_context_json(
    const std::string& ueId, nlohmann::json& out) const;

  bool internal_store_detached_ue_context_json(
    const std::string& ueId, const nlohmann::json& in);

  bool internal_bind_detached_ue_context(const std::string& ueId);

  // Used by HTTP server wiring; backed by amf_config
  bool get_internal_api_enabled() const;
  std::string get_internal_api_token() const;
