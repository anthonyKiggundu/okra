...
// ------------------------- Orchra -------------------------------
void amf_sbi::handle_migration_restore_request(const nlohmann::json& request_body, nlohmann::json& response_body, int& http_code) {
    Logger::amf_sbi().info("Received inter-slice migration request payload from orchestrator");

    try {
        // 1. Verify mandatory snapshot entries are present
        if (!request_body.contains("supi") || !request_body.contains("kseaf") || !request_body.contains("kamf")) {
            response_body["error"] = "Missing mandatory cryptographic parameters in snapshot";
            http_code = 400;
            return;
        }
        ...
        if (request_body.contains("ran_ue_ngap_id")) {
            snap.ran_ue_ngap_id = request_body["ran_ue_ngap_id"].get<uint32_t>();
        }

        // 3. Hand off execution tracking directly to the AMF application engine
        bool restore_ok = amf_app_inst->restore_ue_context_from_snapshot(snap);

        if (restore_ok) {
            response_body["status"] = "MIGRATION_RESTORE_SUCCESS";
            http_code = 200;
        } else {
            response_body["error"] = "Application loop failed to bind security or session descriptors";
            http_code = 500;
        }
    } catch (const std::exception& e) {
        Logger::amf_sbi().error("Exception during migration decoding processing: %s", e.what());
        response_body["error"] = "Internal parsing failure";
        http_code = 500;
    }
}
// --------------------------- end of Orchra ----------------------
