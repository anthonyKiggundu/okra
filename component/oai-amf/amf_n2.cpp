...
// ------------------------ Orchra ----------------------------------
bool amf_n2::restore_ue_ngap_context_from_snapshot(
    const OrchraUeContextSnapshot& snap) {
  Logger::amf_n2().info(
      "Restoring UE NGAP context from snapshot for SUPI %s",
      snap.supi.c_str());

  auto new_ngap_ctx = std::make_shared<ue_ngap_context>();

  // Restore core identifiers
  new_ngap_ctx->amf_ue_ngap_id = snap.amf_ue_ngap_id;
  new_ngap_ctx->ran_ue_ngap_id = snap.ran_ue_ngap_id;

  // Restore association / transport fields that actually exist in ue_ngap_context
  new_ngap_ctx->gnb_assoc_id      = snap.gnb_assoc_id;
  new_ngap_ctx->sctp_stream_recv  = snap.sctp_stream_recv;
  new_ngap_ctx->sctp_stream_send  = snap.sctp_stream_send;
  new_ngap_ctx->ue_context_request = snap.ue_context_request;
  new_ngap_ctx->ncc               = snap.ncc;

  // Optional restoration if present in snapshot
  // new_ngap_ctx->ng_ue_state = snap.ng_ue_state;
  // new_ngap_ctx->tai         = snap.tai;
  new_ngap_ctx->target_ran_ue_ngap_id = snap.target_ran_ue_ngap_id;
  new_ngap_ctx->target_gnb_assoc_id   = snap.target_gnb_assoc_id;

  // Register lookup tables
  /*if (!set_amf_ue_ngap_id_2_ue_ngap_context(
          snap.amf_ue_ngap_id, new_ngap_ctx)) {
    Logger::amf_n2().error(
        "Failed to register restored NGAP context for AMF_UE_NGAP_ID %" PRIu64,
        snap.amf_ue_ngap_id);
    return false;
  }

  if (!set_ran_ue_ngap_id_2_ue_ngap_context(
          snap.gnb_assoc_id, snap.ran_ue_ngap_id, new_ngap_ctx)) {
    Logger::amf_n2().error(
        "Failed to register restored NGAP context for gNB assoc %d / RAN_UE_NGAP_ID %u",
        snap.gnb_assoc_id, snap.ran_ue_ngap_id);
    return false;
  }*/

  set_amf_ue_ngap_id_2_ue_ngap_context(snap.amf_ue_ngap_id, new_ngap_ctx);

  set_ran_ue_ngap_id_2_ue_ngap_context(snap.gnb_assoc_id, snap.ran_ue_ngap_id, new_ngap_ctx);

  Logger::amf_n2().info(
      "Successfully restored NGAP context (AMF_UE_NGAP_ID=%" PRIu64
      ", RAN_UE_NGAP_ID=%u, gNB_ASSOC_ID=%d)",
      snap.amf_ue_ngap_id, snap.ran_ue_ngap_id, snap.gnb_assoc_id);
