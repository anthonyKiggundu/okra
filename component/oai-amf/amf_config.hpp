...
// Orchra added three lines below
constexpr auto AMF_CONFIG_INTERNAL_API         = "internal_api";
constexpr auto AMF_CONFIG_INTERNAL_API_ENABLED = "enabled";
constexpr auto AMF_CONFIG_INTERNAL_API_TOKEN   = "token";
...
struct context_transfer_config {
    bool enabled = false;
...
class amf : public nf {
 private:
  ...
  // Orchra internal API (HTTP/2 internal endpoints)
  bool internal_api_enabled{false};
 ...
 public:
  ...
  [[nodiscard]] bool get_internal_api_enabled() const;
  [[nodiscard]] std::string get_internal_api_token() const;
};
...
class amf_config : public config {
 public:
  ...
  // bool internal_api_enabled{false};
  // std::string internal_api_token{};
  // inside class amf_config (public)

  struct internal_api_t {
    bool enabled{false};
    std::string token{};
  };

  internal_api_t internal_api;
