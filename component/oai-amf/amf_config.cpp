...
void amf::from_yaml(const YAML::Node& node) {
  ...
  for (const auto& elem : node) {
    ...
    if (key == AMF_CONFIG_INTERNAL_API) {
      const YAML::Node& iapi = elem.second;

      if (iapi[AMF_CONFIG_INTERNAL_API_ENABLED]) {
        internal_api_enabled = iapi[AMF_CONFIG_INTERNAL_API_ENABLED].as<bool>();
      }
      if (iapi[AMF_CONFIG_INTERNAL_API_TOKEN]) {
        internal_api_token = iapi[AMF_CONFIG_INTERNAL_API_TOKEN].as<std::string>();
      }

      Logger::config().info(
          "ORCHRA internal_api parsed: enabled=%d token_len=%zu",
          internal_api_enabled, internal_api_token.size());

      continue;  // optional: prevents falling through to other parsing branches
    }

    /* 
    if (key == AMF_CONFIG_INTERNAL_API) {
      auto internal = elem.second;

      if (internal[AMF_CONFIG_INTERNAL_API_ENABLED]) {
        internal_api_enabled =
            internal[AMF_CONFIG_INTERNAL_API_ENABLED].as<bool>();
      }
      if (internal[AMF_CONFIG_INTERNAL_API_TOKEN]) {
        internal_api_token =
            internal[AMF_CONFIG_INTERNAL_API_TOKEN].as<std::string>();
      }
    }
    else if (elem.first.as<std::string>() == AMF_CONFIG_INTERNAL_API) {
      const YAML::Node& iapi = elem.second;
    ...
    Logger::config().info("ORCHRA internal_api parsed: enabled=%d token_len=%zu",
                      internal_api_enabled, internal_api_token.size());
    */
...
// Orchra added two functions below
bool amf::get_internal_api_enabled() const {
  return internal_api_enabled;
}

std::string amf::get_internal_api_token() const {
...
void amf_config::pre_process() {
  ...
  internal_api.enabled = amf_local->get_internal_api_enabled();
  internal_api.token   = amf_local->get_internal_api_token();
  amf_log_level = spdlog::level::from_str(log_level());
 
  // Orchra added two lines below
 // Internal management API (non-3GPP)
  //internal_api_enabled = amf_local->get_internal_api_enabled();
  //internal_api_token   = amf_local->get_internal_api_token();
