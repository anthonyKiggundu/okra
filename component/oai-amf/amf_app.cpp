...
// Orchra extensions run to the bottom
bool amf_application::amf_app::internal_export_ue_context_json(
    const std::string& ueId, nlohmann::json& out) const {

  // 1) Detached store has priority (what POST wrote)
  {
    std::shared_lock lock(m_detached_ue_contexts_mutex);
    const auto it = m_detached_ue_contexts.find(ueId);
    if (it != m_detached_ue_contexts.end()) {
      out = it->second;
      return true;
    }
  }

  // 2) Fallback: live UE context export (treat ueId as SUPI for now)
  std::shared_ptr<ue_context> uc = {};
  if (!supi_2_ue_context(ueId, uc) || !uc) return false;

  out = nlohmann::json::object();
  out["version"] = 1;

  out["ue"] = nlohmann::json::object();
  out["ue"]["supi"] = uc->supi;
  out["ue"]["tmsi"] = uc->tmsi;

  out["ngap"] = nlohmann::json::object();
  out["ngap"]["ran_ue_ngap_id"] = uc->ran_ue_ngap_id;
  out["ngap"]["amf_ue_ngap_id"] = uc->amf_ue_ngap_id;

  out["nas_security"] = nlohmann::json::object();
  out["nas_security"]["present"] = false;

  return true;
}

bool amf_application::amf_app::internal_store_detached_ue_context_json(
    const std::string& ueId, const nlohmann::json& in) {
  std::unique_lock lock(m_detached_ue_contexts_mutex);
  m_detached_ue_contexts[ueId] = in;
  return true;
}

bool amf_application::amf_app::internal_bind_detached_ue_context(
    const std::string& ueId) {
  std::shared_lock lock(m_detached_ue_contexts_mutex);
  const auto it = m_detached_ue_contexts.find(ueId);
  if (it == m_detached_ue_contexts.end()) return false;

  // STUB: Later bind into runtime tables.
  return true;
}

bool amf_application::amf_app::get_internal_api_enabled() const {
  // return amf_cfg->internal_api_enabled;
  return amf_cfg && amf_cfg->internal_api.enabled;
}

std::string amf_application::amf_app::get_internal_api_token() const {
  // return amf_cfg->internal_api_token;
  return (amf_cfg ? amf_cfg->internal_api.token : std::string{});
}
